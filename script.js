...
/* ─────────────────────────────────────────
   OFFICE GALLERY — lazy carousel
───────────────────────────────────────── */
(function initOfficeGallery() {
  const gallery = document.querySelector('.office-gallery');
  if (!gallery) return;

  const mainImg = gallery.querySelector('.office-gallery-image');
  const nextBtn = gallery.querySelector('.office-gallery-next');
  const prevBtn = gallery.querySelector('.office-gallery-prev');
  const currentEl = gallery.querySelector('.office-gallery-current');
  const totalEl = gallery.querySelector('.office-gallery-total');
  const thumbs = Array.from(gallery.querySelectorAll('.office-thumb'));
  ...
  function loadThumb(slide) {
    if (slide.thumbImg && !slide.thumbImg.src) {
      slide.thumbImg.src = slide.thumbImg.dataset.src;
    }
  }

  function loadVisibleThumbs() {
    slides.forEach(loadThumb);
  }
  ...
  function startAutoplay() {
    stopAutoplay();
    autoplayId = window.setInterval(nextSlide, interval);
  }
